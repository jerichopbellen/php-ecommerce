...
$image_sql = "SELECT img_path FROM product_images WHERE product_id = $product_id ORDER BY image_id ASC";
...
<div class="container my-5">
            ...
            <div class="row g-4">
                                ...
                                <?php foreach ($product_images as $index => $img_path): ?>
                                    <div class="carousel-item <?=$index === 0 ? 'active' : '' ?>">
                                        <img src="<?=htmlspecialchars($img_path) ?>" 
                                             ...
                                             onclick="showZoomedImage('<?=htmlspecialchars($img_path) ?>')">
                                    </div>
                                <?php endforeach; ?>
                            </div>
                ...
                <div class="col-md-7">
                    ...
                    <ul class="list-group list-group-flush mb-3">
                        ...
                        <?php endif; ?>
                        <li class="list-group-item"><strong>Total Stock:</strong> <?= $total_stock ?></li>
                    </ul>
                    ...
                    <select name="variant_id" id="variant" class="form-select mb-3" form="addToCartForm" required>
                        <?php foreach ($variants as $v): ?>
                            <option value="<?= htmlspecialchars($v['variant_id']) ?>" <?=$v['quantity'] <= 0 ? 'disabled' : '' ?>>
                                <?=htmlspecialchars("{$v['color']} / {$v['material']}") ?> - ₱<?=number_format($v['price'], 2) ?> (<?= $v['quantity'] > 0 ? $v['quantity'] . ' in stock' : 'Out of stock' ?>)
                            </option>
                        <?php endforeach; ?>
                    </select>
    ...
    <div class="container mb-5">
        <h4 class="mb-4">
            ...
            Reviews (<?= htmlspecialchars($product['review_count']) ?>)
        </h4>
